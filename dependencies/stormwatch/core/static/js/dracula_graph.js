...
Graph.prototype = {
    ...
    addNode: function (id, content, sticky, position) {
        sticky = sticky || false;
        position = position || position;

        // testing if node is already existing in the graph
        if (this.nodes[id] == undefined) {
            this.nodes[id] = new Graph.Node(id, content, sticky, position);
        }
        return this.nodes[id];
    },
 ...
 * Node
 */
Graph.Node = function (id, node, sticky, position) {
    node = node || {};
    sticky = sticky || false; //anv added
    position = position || position; //anv added
    node.id = id;
    node.edges = [];
    node.sticky = sticky;
...
Graph.Renderer.Raphael.prototype = {
    ...
    drawNode: function (node) {
        ...
        if (node.sticky){
            ...
            point = this.translate([node.layoutPosX, node.layoutPosY]);
        }
        ...
        if (!node.render) {
            node.render = function (r, node) {
                ...
                var shape;
                if (node.sticky){
                    shape = r.ellipse(0, 0, 1, 1).attr({fill: color, stroke: color, "stroke-width": 2});
                } else {
                    shape = r.ellipse(0, 0, 30, 20).attr({fill: color, stroke: color, "stroke-width": 2});
                }

                /* set DOM node ID */
                shape.node.id = node.label || node.id;
                shape = r.set().
                    push(shape);
                if (node.label){
                    shape.push(r.text(0, 30, node.label || node.id));
                }
...
Graph.Layout.Spring.prototype = {
    ...
    layoutRepulsive: function (node1, node2) {
        ...
        if (d < this.maxRepulsiveForceDistance) {
            ...
            if (!node1.sticky && !node2.sticky) {
                node2.layoutForceX += repulsiveForce * dx / d;
                node2.layoutForceY += repulsiveForce * dy / d;
                node1.layoutForceX -= repulsiveForce * dx / d;
                node1.layoutForceY -= repulsiveForce * dy / d;
            } else if (node1.sticky && !node2.sticky) {
                node2.layoutForceX += repulsiveForce * dx / d;
                node2.layoutForceY += repulsiveForce * dy / d;
            } else if (node2.sticky && !node1.sticky) {
                node1.layoutForceX -= repulsiveForce * dx / d;
                node1.layoutForceY -= repulsiveForce * dy / d;
            }
        }
